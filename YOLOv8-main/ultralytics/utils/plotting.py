...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        Args:
            masks (tensor): Predicted Annotations on cuda, shape: [n, h, w]
            colors (List[List[Int]]): Colors for predicted Annotations, [[r, g, b] * n]
            im_gpu (tensor): Image is in cuda, shape: [3, h, w], range: [0, 1]
            alpha (float): Mask transparency: 0.0 fully transparent, 1.0 opaque
            retina_masks (bool): Whether to use high resolution Annotations or not. Defaults to False.
        """
        if self.pil:
