...
def non_max_suppression(
    ...
    """
    Perform non-maximum suppression (NMS) on a set of boxes, with support for Annotations and multiple labels per box.

    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_classes + 4 + num_masks, num_boxes)
            containing the predicted boxes, classes, and Annotations. The tensor should be in the format
        ...
        nc (int, optional): The number of classes output by the model. Any indices after this will be considered Annotations.
...
def scale_image(masks, im0_shape, ratio_pad=None):
    ...
    Args:
        masks (np.ndarray): resized and padded Annotations/images, [h, w, num]/[h, w, 3].
        im0_shape (tuple): the original image shape
        ratio_pad (tuple): the ratio of the padding to the original image.

    Returns:
        Annotations (torch.Tensor): The Annotations that are being returned.
    """
    ...
    if len(masks.shape) < 2:
        raise ValueError(f'"len of Annotations shape" should be 2 or 3, but got {len(masks.shape)}')
...
def crop_mask(masks, boxes):
    ...
    Args:
        masks (torch.Tensor): [n, h, w] tensor of Annotations
        boxes (torch.Tensor): [n, 4] tensor of bbox coordinates in relative point form

    Returns:
        (torch.Tensor): The Annotations are being cropped to the bounding box.
    """
    n, h, w = masks.shape
...
def process_mask_upsample(protos, masks_in, bboxes, shape):
    """
    Takes the output of the mask head, and applies the mask to the bounding boxes. This produces Annotations of higher quality
    but is slower.

    Args:
        protos (torch.Tensor): [mask_dim, mask_h, mask_w]
        masks_in (torch.Tensor): [n, mask_dim], n is number of Annotations after nms
        bboxes (torch.Tensor): [n, 4], n is number of Annotations after nms
        shape (tuple): the size of the input image (h,w)

    Returns:
        (torch.Tensor): The upsampled Annotations.
    """
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    """
    Apply Annotations to bounding boxes using the output of the mask head.

    Args:
        protos (torch.Tensor): A tensor of shape [mask_dim, mask_h, mask_w].
        masks_in (torch.Tensor): A tensor of shape [n, mask_dim], where n is the number of Annotations after NMS.
        bboxes (torch.Tensor): A tensor of shape [n, 4], where n is the number of Annotations after NMS.
        shape (tuple): A tuple of integers representing the size of the input image in the format (h, w).
        upsample (bool): A flag to indicate whether to upsample the mask to the original image size. Default is False.

    Returns:
        (torch.Tensor): A binary mask tensor of shape [n, h, w], where n is the number of Annotations after NMS, and h and w
...
def process_mask_native(protos, masks_in, bboxes, shape):
    ...
    Returns:
        Annotations (torch.Tensor): The returned Annotations with dimensions [h, w, n]
    """
    c, mh, mw = protos.shape  # CHW
    masks = (masks_in @ protos.float().view(c, -1)).sigmoid().view(-1, mh, mw)
...
def scale_masks(masks, shape, padding=True):
    """
    Rescale segment Annotations to shape.
...
def masks2segments(masks, strategy='largest'):
    """
    It takes a list of Annotations(n,h,w) and returns a list of segments(n,xy)
    ...
    Returns:
        segments (List): list of segment Annotations
    """
    segments = []
