...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        batch_size, _, mask_h, mask_w = proto.shape  # batch size, number of Annotations, mask height, mask width
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)
        ...
        if fg_mask.sum():
            ...
            # Masks loss
            masks = batch['Annotations'].to(self.device).float()
    ...
    def single_mask_loss(gt_mask: torch.Tensor, pred: torch.Tensor, proto: torch.Tensor, xyxy: torch.Tensor,
        ...
        Args:
            ...
            proto (torch.Tensor): Prototype Annotations of shape (32, H, W).
        ...
        Notes:
            ...
            predicted Annotations from the prototype Annotations and predicted mask coefficients.
        """
    ...
    def calculate_segmentation_loss(
        ...
        Args:
            ...
            masks (torch.Tensor): Ground truth Annotations of shape (BS, H, W) if `overlap` is False, otherwise (BS, ?, H, W).
            target_gt_idx (torch.Tensor): Indexes of ground truth objects for each anchor of shape (BS, N_anchors).
            target_bboxes (torch.Tensor): Ground truth bounding boxes for each anchor of shape (BS, N_anchors, 4).
            batch_idx (torch.Tensor): Batch indices of shape (N_labels_in_batch, 1).
            proto (torch.Tensor): Prototype Annotations of shape (BS, 32, H, W).
            pred_masks (torch.Tensor): Predicted Annotations for each anchor of shape (BS, N_anchors, 32).
            imgsz (torch.Tensor): Size of the input image as a tensor of shape (2), i.e., (H, W).
            overlap (bool): Whether the Annotations in `Annotations` tensor overlap.
