...
class FastSAMPrompt:
    ...
    def plot(self,
        ...
        Args:
            ...
            mask_random_color (bool, optional): Whether to use random color for Annotations. Defaults to True.
    ...
    def fast_show_mask(
        ...
        Args:
            ...
            random_color (bool, optional): Whether to use random color for Annotations. Defaults to False.
            bbox (list, optional): Bounding box coordinates [x1, y1, x2, y2]. Defaults to None.
            points (list, optional): Points to be plotted. Defaults to None.
    ...
    def box_prompt(self, bbox):
        """Modifies the bounding box properties and calculates IoU between Annotations and bounding box."""
        if self.results[0].masks is not None:
            ...
            # IoUs = torch.zeros(len(Annotations), dtype=torch.float32)
    ...
    def point_prompt(self, points, pointlabel):  # numpy
        """Adjusts points on detected Annotations based on user input and returns the modified results."""
        if self.results[0].masks is not None:
            if os.path.isdir(self.source):
                raise ValueError(f"'{self.source}' is a directory, not a valid source for this function.")
