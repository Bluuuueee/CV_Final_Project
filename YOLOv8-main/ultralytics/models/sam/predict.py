...
class Predictor(BasePredictor):
    ...
    points, and low-resolution Annotations.
    ...
    def inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False, *args, **kwargs):
        ...
        Args:
            ...
            masks (np.ndarray, optional): Low-resolution Annotations from previous predictions. Shape should be (N, H, W). For SAM, H=W=256.
            multimask_output (bool, optional): Flag to return multiple Annotations. Helpful for ambiguous prompts. Defaults to False.

        Returns:
            (tuple): Contains the following three elements.
                - np.ndarray: The output Annotations in shape CxHxW, where C is the number of generated Annotations.
        ...
        masks = self.prompts.pop('Annotations', masks)
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        """
        Internal function for image segmentation inference based on cues like bounding boxes, points, and Annotations.
    ...
    def generate(self,
        ...
        Args:
            ...
            crop_nms_thresh (float): IoU cutoff for Non-Maximum Suppression (NMS) to remove duplicate Annotations between crops.

        Returns:
            (tuple): A tuple containing segmented Annotations, confidence scores, and bounding boxes.
        """
        ...
        for crop_region, layer_idx in zip(crop_regions, layer_idxs):
            ...
            for (points, ) in batch_iterator(points_batch_size, points_for_image):
                ...
                # Interpolate predicted Annotations to input size
        ...
        # Remove duplicate Annotations between crops
    ...
    def postprocess(self, preds, img, orig_imgs):
        """
        Post-processes SAM's inference outputs to generate object detection Annotations and bounding boxes.

        The method scales Annotations and boxes to the original image size and applies a threshold to the mask predictions. The
        SAM model uses advanced architecture and promptable segmentation tasks to achieve real-time performance.

        Args:
            preds (tuple): The output from SAM model inference, containing Annotations, scores, and optional bounding boxes.
            img (torch.Tensor): The processed input image tensor.
            orig_imgs (list | torch.Tensor): The original, unprocessed images.

        Returns:
            (list): List of Results objects containing detection Annotations, bounding boxes, and other metadata.
        """
        # (N, 1, H, W), (N, 1)
    ...
    def remove_small_regions(masks, min_area=0, nms_thresh=0.7):
        """
        Perform post-processing on segmentation Annotations generated by the Segment Anything Model (SAM). Specifically, this
        function removes small disconnected regions and holes from the input Annotations, and then performs Non-Maximum
        Suppression (NMS) to eliminate any newly created duplicate boxes.

        Args:
            masks (torch.Tensor): A tensor containing the Annotations to be processed. Shape should be (N, H, W), where N is
                                  the number of Annotations, H is height, and W is width.
        ...
        Returns:
            (tuple([torch.Tensor, List[int]])):
                - new_masks (torch.Tensor): The processed Annotations with small regions removed. Shape is (N, H, W).
                - keep (List[int]): The indices of the remaining Annotations post-NMS, which can be used to filter the boxes.
        """
        if len(masks) == 0:
            return masks
        ...
        for mask in masks:
            ...
            # Give score=0 to changed Annotations and 1 to unchanged Annotations so NMS prefers Annotations not needing postprocessing
